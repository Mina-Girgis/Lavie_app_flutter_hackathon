...
import 'package:lavie/screens/forums/components/companents.dart';
...
class DiscussionForumsScreen extends StatelessWidget {
  DiscussionForumsScreen({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(
      child: BlocConsumer<ForumsCubit, ForumsState>(
        ...
        builder: (context, state) {
          return Scaffold(
            floatingActionButton: FloatingActionButton(
              onPressed: (){
                cubit.changeaddPhotoIndex(0);
                cubit.changePhotoBase64("");
                AppNavigator.customNavigator(context: context, screen: AddForumScreen(), finish: false);
                  ...
                  child: Column(
                    children: [
                      defaultSearchBar(
                        ...
                        readOnly: false ,
                        onChanged: (value){
                          cubit.getSearchForumsData(value.toString());
                          // print(cubit.searchForumsData.length);
                        },
                      ),
                      SizedBox(height: SizeConfig.defaultSize!*2,),
                      ConditionalBuilder(
                          condition: (cubit.searchForumsData.isNotEmpty),
                          builder: (context){
                            return Column(
                              children: [
                                Container(
                                  ...
                                  child: ListView.separated(
                                    ...
                                    itemBuilder: (context,index){
                                      return InkWell(
                                        onTap: (){
                                          cubit.changeTitelsIndex(index);
                                        },
                                        child: Container(
                                          width: SizeConfig.defaultSize!*12,
                                          decoration: BoxDecoration(
                                            color: (cubit.titelsIndex==index)?GREEN:SEARCH_BAR_COLOR,
                                          ...
                                          child: Center(child: Text(
                                            titles[index],
                                            style: TextStyle(
                                                color: (cubit.titelsIndex==index)?Colors.white:GRAY,
                                                fontSize: 16.0
                                            ),
                                          )),
                                        ),
                                    ...
                                    return SizedBox(width: SizeConfig.defaultSize!*2,);
                                  },
                                  ),
                                ),
                                SizedBox(height: SizeConfig.defaultSize!*2,),
                                ListView.separated(
                                  ...
                                  itemCount: cubit.searchForumsData.length,
                                  separatorBuilder: (context,index){
                                    return SizedBox(height: SizeConfig.defaultSize!*2);
                                  },
                                  itemBuilder: (context,index){
                                    return forumDesign(model:cubit.searchForumsData[index]);
                                  },
